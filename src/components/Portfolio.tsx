...
const projects = [
    ...
    },
    {
        ...
        image: "https://images.unsplash.com/photo-1507146426996-ef05306b995a?auto=format&fit=crop&q=80&w=300",
        ...
        <Box className="bg-gray-50 py-12 px-4 shadow-inner">
            ...
            <Box className="space-y-6">
                {projects.map((project, index) => (
                    <Box key={index} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 flex flex-col items-center">
                        <img src={project.image} alt={project.title} className="w-full h-48 object-cover" />
                        <Box className="p-5 w-full">
                            <Text className="text-primary text-[10px] font-bold uppercase mb-1">{project.category}</Text>
                            <Text className="font-bold text-gray-900 text-lg leading-tight mb-2">{project.title}</Text>
                            <Text className="text-gray-600 text-sm italic">{project.desc}</Text>
                        </Box>
                    </Box>
                ))}
            </Box>
